...
const get = async (basedir) => {
    ...
    await fs
        ...
        .then(res => files.push(...((res?.filter(Boolean) ?? []).map(file => join(`config`, file)) ?? [])));
    if (!files?.length)
        return;
...
async function fetchFileInfo(filename) {
    ...
    if (!inspect?.name || inspect.type === `dir` || !inspect.absolutePath) {
        logger.info(`Skipping`, inspect?.name, inspect?.type, inspect?.absolutePath);
        return;
    }
    const file = {
    ...
    if (file.dynamic && file.file) {
        ...
        const tmpPath = join(tmpDir, `${file.sha1}${uid}${file.parsed.ext.replace(`ts`, `js`)}`);
        const cachePath = join(paths.storage, `conf`, `${file.sha1}${file.parsed.ext.replace(`ts`, `js`)}`);
        try {
                ...
                try {
                    ...
                    if ([`.ts`, `.cts`, `.mts`].includes(file.parsed.ext)) {
                        if (!(await fs.exists(cachePath))) {
                            logger
                                .scope(`fs`)
                                .log(`${file.name} does not exist in cache. running transform.`, `file will be cached to ${cachePath}`);
 ...
 */
async function transformConfig({ file, cachePath, }) {
    logger.time(`compiling ${file.name}`);
    if (!transformer) {
        transformer = await import(`../esbuild/index.js`).then(async ({ getImplementation }) => await getImplementation(file.path));
    }
 ...
 */
async function handleBudError(ErrorClass, cause, file) {
    if (typeof file.name !== `string`)
        return;
    if (typeof file.dynamic !== `boolean`)
        return;
    /**
...
function getFileType(file) {
    if (file.name?.includes(`production`))
        return `production`;
    if (file.name?.includes(`development`))
        return `development`;
    return `base`;
}
export { get, data };
