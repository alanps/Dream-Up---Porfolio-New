...
async function getFileInfo(filename) {
    ...
    if (!isNormalInspectResult(inspect))
        return;
    ...
    if (file.type === `module`) {
        file.module = async () => {
            ...
            const outfile = join(paths.storage, `configs`, file.base, `${current.sha1}${file.ext.replace(/(.*)ts$/, `$1mjs`)}`);
            ...
            const tmpfile = join(paths.basedir, file.dir, `.${file.name}${file.ext.replace(/(.*)ts$/, `$1mjs`)}`);
...
async function esTransform({ file, outfile, }) {
    if (!transformer) {
        transformer = await import(`@roots/bud-support/esbuild`)
            .then(async ({ getImplementation }) => await getImplementation(file.path))
            .catch(error => {
            throw error;
        });
    }
...
function isNormalInspectResult(file) {
    if (file === undefined)
        return false;
    ...
    if (file.name === undefined)
        return false;
    if (file.absolutePath === undefined)
        return false;
    if ([`dir`, false, undefined].includes(file.type))
        return false;
    return true;
}
const makeTmpFileImportError = (file) => async (error) => {
    await fs.remove(file).catch(makeRemoveError(file));
    throw error;
};
const makeRemoveError = (file) => (error) => {
    logger.scope(`fs`).error(`error removing file`, file, error);
    throw error;
};
const makeCopyError = (file) => async (error) => {
