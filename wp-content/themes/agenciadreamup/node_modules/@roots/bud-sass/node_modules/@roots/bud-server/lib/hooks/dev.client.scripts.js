 ...
 * @returns string
 */
export const proxyClickInterceptor = (app) => {
    if (!app.hooks.filter(`dev.middleware.enabled`, []).includes(`proxy`))
        return;
    const params = new URLSearchParams({
        search: app.hooks.filter(`dev.middleware.proxy.options.target`, new URL(`http://0.0.0.0`))?.href,
        replace: `/`,
    });
 ...
 * @returns string
 */
export const hmrClient = (app) => {
    if (app.isCLI() && app.context.args.hot === false)
        return;
    const params = new URLSearchParams({
        name: app.label,
        indicator: !app.isCLI() ||
