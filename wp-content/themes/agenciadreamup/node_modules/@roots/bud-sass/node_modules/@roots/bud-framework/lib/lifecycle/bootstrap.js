 ...
 * @returns filter fn
 */
const filterServices = (app) => (signifier) => (app.isDevelopment || !DEVELOPMENT_SERVICES.includes(signifier)) &&
    (app.isRoot || !PARENT_SERVICES.includes(signifier));
/**
 * Import and bind framework services
 */
const instantiateServices = (app) => async (signifier) => {
    ...
    app.log(chalk.blue(`bud.${label}`), figures.arrowLeft, chalk.cyan(signifier));
    app.services.push(label);
};
const initializeCoreUtilities = (bud) => {
    bud.fs = new FS(() => bud);
...
export const bootstrap = async function (context) {
    ...
    Object.entries(LIFECYCLE_EVENT_MAP).map(([eventHandle, callbackName]) => this.services
        ...
        .map(([label, service]) => {
        if (!service) {
            this.context.logger.error(`service not found: ${label}`, this.services);
        }
        if (!isFunction(service[callbackName]))
            return;
        this.hooks.action(eventHandle, service[callbackName].bind(service));
        this.context.logger.info(`${label}.${callbackName}`, `bound to`, eventHandle);
    }));
    await [
        `init`,
        `bootstrap`,
    ...
    this.after(async (bud) => {
        ...
        await this.fs.write(this.path(`@storage`, `checksum.yml`), Object.entries(this.context.files).reduce((acc, [key, { sha1 }]) => (!sha1 ? acc : { ...acc, [key]: sha1 }), {}));
