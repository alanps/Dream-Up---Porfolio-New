...
export default async (context) => {
    ...
    let env = projectEnv.get(paths.basedir);
    let bud = await budContext.get(fs);
    try {
    ...
    let files = await projectFiles.get(paths.basedir);
    Object.entries(files).map(([k, v]) => logger.scope(`bootstrap`).info(`file`, k, v));
