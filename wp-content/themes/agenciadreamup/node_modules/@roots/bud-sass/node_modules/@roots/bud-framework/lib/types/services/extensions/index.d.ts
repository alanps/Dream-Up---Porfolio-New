...
export type LifecycleMethods = 'register' | 'boot' | 'configAfter' | 'buildBefore' | 'buildAfter' | 'make';
/**
...
export interface Service extends BaseService {
    ...
    instantiate<K extends `${keyof Modules & string}`>(extension: (new (...args: any[]) => Modules[K]) | Extension, options?: Record<string, any>): Promise<Extension>;
     ...
     * Add an extension
     */
    add(extension: ExtensionLiteral | Extension | (new (bud: Bud) => ExtensionLiteral) | `${keyof Modules & string}` | Array<ExtensionLiteral | Extension | (new (bud: Bud) => ExtensionLiteral) | `${keyof Modules & string}`>): Promise<void>;
    import<K extends `${keyof Modules}`>(signifier: K, fatalOnError?: boolean): Promise<Extension>;
    runAll(methodName: LifecycleMethods): Promise<Array<void>>;
    run<K extends `${keyof Modules & string}`>(extension: Extension | K, methodName: LifecycleMethods): Promise<this>;
    /**
     * Run a lifecycle method on all dependencies of a given extension or
     * extension signifier.
     */
    runDependencies<K extends `${keyof Modules & string}`>(extension: Extension | K, methodName: LifecycleMethods): Promise<void>;
    /**
