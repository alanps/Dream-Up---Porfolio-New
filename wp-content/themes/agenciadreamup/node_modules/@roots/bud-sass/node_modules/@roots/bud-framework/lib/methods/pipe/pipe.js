...
export const pipe = async function (functions, maybeInitialValue) {
    const initialValue = Promise.resolve(!isUndefined(maybeInitialValue) ? maybeInitialValue : this);
    return await functions.reduce(async (value, fn) => await fn(await value), initialValue);
};
