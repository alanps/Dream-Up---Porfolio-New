 ...
 * @returns Promise
 */
export const initialize = (bud) => bud.hooks
    .fromMap({
    'feature.hash': () => false,
})
    .hooks.fromMap({
    ...
    .hooks.fromMap({
    'location.@src': bud.isCLI() && isString(bud.context.args.input)
        ? bud.context.args.input
        : `src`,
    'location.@dist': bud.isCLI() && isString(bud.context.args.output)
        ? bud.context.args.output
        : `dist`,
    'location.@storage': paths.get(bud.context.basedir)[`storage`],
    'location.@modules': bud.isCLI() && isString(bud.context.args.modules)
    ...
    .when(bud.isDevelopment, ({ hooks }) => hooks.fromMap({
