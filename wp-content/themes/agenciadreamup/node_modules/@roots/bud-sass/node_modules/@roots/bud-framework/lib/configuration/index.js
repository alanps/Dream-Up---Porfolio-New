 ...
 */
export const process = async (app) => {
    const findConfigs = (ofType, isLocal) => sortBy(configs
    ...
    await Promise.all(findConfigs(`base`, false).map(async (description) => {
    ...
    await Promise.all(findConfigs(`base`, true).map(async (description) => {
    ...
    await Promise.all(findConfigs(app.mode, false).map(async (description) => {
    ...
    await Promise.all(findConfigs(app.mode, true).map(async (description) => {
        app.log(`processing ${app.mode} local configuration`, description.name);
        try {
            await configuration.run(description);
        }
        catch (err) {
    ...
    if (app.hasChildren) {
        await Promise.all(Object.values(app.children).map(async (child) => {
            try {
