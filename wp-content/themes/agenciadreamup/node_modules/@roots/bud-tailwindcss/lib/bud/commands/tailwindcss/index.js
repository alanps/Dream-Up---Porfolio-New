import { __decorate } from "tslib";
import { join } from 'node:path';
import BudCommand from '@roots/bud/cli/commands/bud';
import { dry } from '@roots/bud/cli/decorators/command.dry';
import { Option } from '@roots/bud-support/clipanion';
let BudTailwindCommand = class BudTailwindCommand extends BudCommand {
    constructor() {
        super(...arguments);
        this.options = Option.Proxy({ name: `tailwindcss passthrough options` });
    }
    async execute() {
        await this.makeBud(this);
        await this.bud.run();
        const tw = join(await this.bud.module.getDirectory(`tailwindcss`), `lib`, `cli.js`);
        if (!this.options.length) {
            const input = await Promise.all(await this.bud.fs.find(this.bud.path(`@src`, `**/*.css`))).then(paths => paths.filter(Boolean).shift());
            this.options.push(`--input`, input, `--output`, this.bud.relPath(this.bud.path(`@dist`, this.bud.relPath(`@src`, input))));
            const config = this.bud.context.files
                ? Object.values(this.bud.context.files)?.find(file => file.name.includes(`tailwind.config`))
                : false;
            if (config && config?.path) {
                this.options.push(`--config`, this.bud.relPath(config.path));
            }
        }
        this.context.stdout.write(`tailwindcss ${this.options.join(` `)}\n\n`);
        await this.$(this.bin, [tw, ...(this.options ?? [])]);
    }
};
BudTailwindCommand.paths = [[`tailwindcss`], [`tw`]];
BudTailwindCommand.usage = BudCommand.Usage({
    category: `tools`,
    description: `tailwindcss CLI passthrough`,
    examples: [
        [`View tailwindcss usage information`, `$0 tailwindcss --help`],
    ],
});
BudTailwindCommand = __decorate([
    dry
], BudTailwindCommand);
export { BudTailwindCommand };
//# sourceMappingURL=index.js.map